...
static signed char finetune_signed(unsigned char raw) {
    if (raw >= 8) return (signed char)((signed char)raw - 16);
...
static unsigned period_finetune(unsigned period, unsigned char finetune_raw) {
    ...
    if (ft == 0 || period == 0) return period;
...
static void ua_update_channel(char ch) {
    ...
    ua_vol = (unsigned char)(
        (unsigned)c->volume * (unsigned)modplay.master_volume / 64);
...
static unsigned char wave_value(unsigned char wave, unsigned char pos) {
    ...
    switch (wave & 0x03) {
    case 0: return vib_sine[idx];               // sine
    case 1: return (unsigned char)(255 - idx * 8); // ramp down
    case 2: return 255;                         // square
    default: return (unsigned char)(idx * 8);  // ramp up (random approx)
    }
}
...
static void apply_note(char ch_idx, note_cell_t *n) {
    ...
    if (n->period != 0 && n->effect != FX_PORTA_TO_NOTE) {
        unsigned char ft = (c->sample_num >= 1) ?
            modplay.sample[c->sample_num - 1].finetune : 0;
...
static void fx_row(char ch_idx) {
    ...
    case FX_PORTA_UP:
    case FX_PORTA_DOWN:
        if (c->eparam) c->porta_speed = c->eparam;
        break;
    ...
    case FX_VIBRATO:
        if (lo) c->vib_depth = lo;
        if (hi) c->vib_speed = hi;
        break;
    ...
    case FX_TREMOLO:
        if (lo) c->trm_depth = lo;
        if (hi) c->trm_speed = hi;
...
static void fx_tick(char ch_idx) {
    ...
    case FX_TREMOLO: {
        ...
        unsigned char delta = (unsigned char)(
            (unsigned)val * c->trm_depth >> 6);
        if (!half)
            c->volume = clamp_vol(
...
static void process_new_row(void) {
    ...
    row_reu = modplay.reu_base
            + MOD_HDR_PATTERNS
            + (unsigned long)modplay.pattern_num * 1024UL
            + (unsigned long)modplay.row * MOD_BYTES_PER_ROW;
...
static void advance_position(void) {
    ...
    if (modplay.skip_row_advance) {
        ...
        if (modplay.order_pos >= modplay.song_length) {
            if (modplay.loop_song) modplay.order_pos = 0;
            else { modplay.active = 0; return; }
        }
...
void modplay_tick(void) {
    ...
    if (!modplay.active || mod_paused) return;
...
// ---------------------------------------------------------------
void __asm modplay_irq_entry(void) {
...
char modplay_load(char *filename, unsigned long reu_addr) {
    ...
    // Load entire file into REU at reu_addr
    // Using the UCI REU load command (loads open file into REU)
    // The reu_addr field in the UCI load_reu command uses the
    // REU base address registers in the UCI command structure.
    // Here we use a direct UCI approach: read chunks via uii_read_file
    // and copy to REU using the REU stash command.
    // For simplicity, use uii_load_reu after navigating the file.
    uii_close_file();

    // Re-open and use UCI load-into-REU command (fastest path)
    uii_open_file(0x01, filename);
    if (!UII_SUCCESS) return 0;

    // Determine REU size class that covers the file; use index 7 (16MB max)
    uii_load_reu(7);
    uii_close_file();
...
char modplay_init(unsigned long reu_addr) {
    ...
    p = &hdr[MOD_HDR_MAGIC];
    if (p[0]=='M' && p[1]=='.' && p[2]=='K' && p[3]=='.') modplay.num_samples = 31;
    else if (p[0]=='M' && p[1]=='!' && p[2]=='K' && p[3]=='!') modplay.num_samples = 31;
    else if (p[0]=='4' && p[1]=='C' && p[2]=='H' && p[3]=='N') modplay.num_samples = 31;
    else if (p[0]=='F' && p[1]=='L' && p[2]=='T' && p[3]=='4') modplay.num_samples = 31;
    else {
        // Check for 15-sample format: magic area contains printable ASCII
        unsigned char ok = 1;
        for (i = 0; i < 4; i++) {
            if (p[i] < 0x20 || p[i] >= 0x7F) { ok = 0; break; }
        }
        if (!ok) return 0;
    ...
    if (modplay.num_samples == 31)
        sample_data_base = reu_addr + MOD_HDR_PATTERNS
                         + (unsigned long)modplay.num_patterns * 1024UL;
    else
        // 15-sample: header is 20+15*30+2+128 = 600 bytes (no magic)
        sample_data_base = reu_addr + 600UL
                         + (unsigned long)modplay.num_patterns * 1024UL;
    ...
    for (i = 0; i < modplay.num_samples; i++) {
        ...
        unsigned long len_words = ((unsigned long)sh[MSMP_LEN_HI] << 8)
                                 | (unsigned long)sh[MSMP_LEN_LO];
        unsigned long len_bytes = len_words * 2;

        unsigned long rep_words = ((unsigned long)sh[MSMP_REPA_HI] << 8)
                                 | (unsigned long)sh[MSMP_REPA_LO];
        unsigned long replen_w  = ((unsigned long)sh[MSMP_REPLEN_HI] << 8)
                                 | (unsigned long)sh[MSMP_REPLEN_LO];
    ...
    for (i = 0; i < MOD_CHANNELS; i++) {
        modplay.channel[i].pan =
            (modplay.stereo && (i == 1 || i == 2)) ?
                MOD_PAN_RIGHT : MOD_PAN_LEFT;
    }
